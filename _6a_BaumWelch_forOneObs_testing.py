...
def BaumWelchOneObs(obs, priorPi, przejsciaM, emisjaE, steps = 10, shouldPrint=False):
    ...
    while step < steps:
        ...
        alfa, prObsAlfa = forward( obs, pi, przejscia, emisja )
        alfa, alfa_denom = normalizedProbMany( alfa )
        beta, prObsBeta = backward( obs, pi, przejscia, emisja )
        ...
        emisja2 = np.array( [[ sum( identity( o, O[t] ) * eta[t][i] for t in range(1,T+1)  # range(T) -> range(1,T+1)
                                   ) / sum( eta[t][i] for t in range(1,T+1) )   # range(T) -> range(1,T+1)
...
if __name__ == '__main__':
    ...
    obs = [2, 2, 2, 1, 2, 3, 4, 4, 0, 3, 4, 1, 3, 2, 0, 0, 1, 1, 0, 2, 2, 2, 2, 2, 4, 2, 3, 1, 0, 0, 1, 4, 2, 4, 0, 1, 2, 0, 2, 2, 3, 1, 4, 3, 3, 4, 2, 1, 2, 0, 3, 2, 0, 2, 2, 2, 1, 2, 0, 1, 2, 1, 0, 0, 4, 3, 2, 2, 3, 2, 3, 2, 3, 4, 2, 0, 2, 2, 2, 0, 1, 2, 0, 1, 1, 3, 0, 2, 1, 3, 2, 2, 2, 3, 0, 0, 1, 2, 4, 2, 0, 3, 2, 2, 4, 4, 2, 4, 2, 0, 0, 2, 1, 1, 3, 2, 2, 3, 1, 0, 0, 3, 3, 4, 2, 0, 2, 3, 2, 3, 1, 4, 0, 2, 4, 1, 1, 4, 2, 2, 4, 4, 1, 4, 4, 4, 0, 2, 4, 2, 4, 4, 3, 4, 1, 0, 2, 0, 2, 2, 2, 4, 4, 1, 0, 4, 4, 1, 4, 2, 1, 0, 2, 2, 3, 1, 3, 2, 3, 0, 2, 0, 1, 2, 2, 2, 2, 1, 2, 4, 3, 3, 4, 2, 3, 3, 2, 0, 2, 4, 0, 1, 4, 2, 4, 2, 1, 2, 1, 4, 3, 1, 2, 3, 0, 0, 4, 3, 1, 0, 2, 3, 1, 1, 4, 4, 4, 4, 2, 1, 2, 4, 0, 0, 2, 0, 2, 0, 4, 4, 2, 2, 2, 0, 2, 1, 1, 2, 4, 2, 3, 0, 4, 2, 2, 2, 4, 4, 2, 0, 2, 2, 1, 1, 1, 3, 2, 1, 1, 1, 1, 4, 0, 3, 3, 1, 1, 2, 2, 2, 1, 2, 3, 4, 0, 2, 4, 4, 3, 4, 1, 2, 2, 1, 4, 0, 3, 3, 2, 4, 4]
